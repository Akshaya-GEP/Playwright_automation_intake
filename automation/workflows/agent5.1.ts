 ...
 */
export async function workflowAgent5_TC2(page: Page, _ctx: AgentContext) {
  const env = getEnv();
  ...
  // Access process.env directly or use the updated env property (if env.ts is updated)
  const updateType = process.env.UPDATE_TYPE_5_2 || 'Exception Approval Required';
  const reasonAction = process.env.REASON_ACTION_5_2 || 'Approve supplier';
  // ------------------------------
  ...
  // Agent 5 flow shows a supplier summary screen with "Proceed with Request" after grid selection.
  // Agent 5.1 should follow the same behavior.
  ...
  // Use a visible-scoped locator to avoid hidden/diagnostics matches.
  ...
  await expect(updatePrompt).toBeVisible({ timeout: 180_000 });
  ...
  // After selecting "Choose Option(s)", the CTA is typically "Proceed" (not "Proceed with Request").
  // Some builds may still show "Proceed with Request"; click whichever is enabled.
  ...
  const clicked = await clickFirstEnabled(
    page,
    [
      { name: 'Proceed', locator: proceedBtn },
      { name: 'Proceed with Request', locator: proceedWithRequest },
    ],
    { timeoutMs: 60_000 },
  );
  ...
  if (clicked) {
    ...
    console.warn('No enabled CTA found after selecting update type; continuing...');
  }
  ...
  const hasUploadPrompt = await uploadPrompt.first().isVisible({ timeout: 60_000 }).catch(() => false);
